...
describe('I. Documents filtering', () => {
  it.only('1. .. using Go=4, JS=2 and Android=1, should get at least 3 devs' +
    ...
    cy.wait(150)
    cy.get("div.menuable__content__active div:nth-child(4)").click()
...
describe('II. Inserting and deleting a dev', () => {
...
describe('III. Editing a dev', () => {
  it('1. .. should show modified data of a dev', () => {
    ...
    cy.get("[data-test='first']").clear().type('Mickey')
    cy.get("[data-test='last']").clear().type('Mouse')

    cy.get('div:nth-child(10) div:nth-child(5) a div div').click({ force: true})  // Architect

    cy.get("[data-test='email']").clear().type('Mickey@Mouse.pl')
    cy.get("[data-test='btnSave']").click({ force: true })
    cy.get("[data-test='find']").type('Mouse')
    cy.get("[data-test='btnFind']").click()

    cy.get(tableCell(1, 2)).contains('Architect').should('exist')
    cy.get(tableCell(1, 3)).contains('Mickey').should('exist')
    cy.get(tableCell(1, 4)).contains('Mouse').should('exist')
    cy.get('tr:nth-child(1)').find('[data-test="edit"]').click()

    cy.get("[data-test='first']").clear().type('Betty')
    cy.get("[data-test='last']").clear().type('Cram')

    cy.get('div:nth-child(10) div:nth-child(3) a div div').click({ force: true})  // Senior

    cy.get("[data-test='email']").clear().type('betty.cram@gmail.com')
    cy.get("[data-test='btnSave']").click({ force: true })
